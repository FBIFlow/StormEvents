...
public class StormEvent extends Event {
    ...
    public StormEvent() {
        super();
        //предопределенные задачи желательно регистрировать в конструкторе, чтобы EventProcessor мог корректно начать
        //выполнение задачи, приняв наименьшее значение времени выполнения за начальное состояние счётчика(если мы хотим
        //чтобы первоначальное значение счётчика было меньше 0)
        //так-же задачи можно регистировать во время выполнения ивента

        //поля класса в основном стоит инициализировать в onInit(), потому-что при регистрации события в EventHolder создаётся его
        //экземпляр.
        registerTask(() -> {
    ...
    @Override
    public String getId() {
        //id события, аналогичный id нужно использовать в файле конфигурации, чтобы зарегистрировать ивент и передать в
        //него какие-либо данные
        return "storm-event";
    }

    @Override
    public int getLiveTimeTicks() {
        //то, сколько ивент будет существовать, прежде чем будет вызван onRemove()
    ...
    public void onInit() {
        ...
        Bukkit.getOnlinePlayers().stream().forEach(x -> x.sendMessage("LOCATION IS: " + location));

        triangle = new Triangle(ofLocation(location.clone().add(25, 0, 0).clone()), ofLocation(location.clone().add(-20, 0, -25).clone()), ofLocation(location.clone().add(-20, 0, 25).clone()));

        Bukkit.getOnlinePlayers().stream().forEach(x -> x.sendMessage(String.format("TRIANGLE IS: %s %s %s", triangle.A, triangle.B, triangle.C)));
        System.err.println("TRIANGLE IS: " + triangle);
        bigTriangle = scaleTriangle(triangle, 15);
        System.err.println("BIG TRIANGLE IS: " + bigTriangle);
        eventListener = new EventListener(this);
        System.out.println("called onInit");
        Set<String> holoLineKeys = dataContainer.getKeys().stream().filter(item -> item.contains("holo-line-")).collect(Collectors.toSet());
        holoLineKeys.forEach(key -> {
            String line = (String) dataContainer.get(key);
            hologramLines.add(line);
        });

        Set<String> lootKeys = dataContainer.getKeys().stream().filter(item -> item.contains("loot-item-")).collect(Collectors.toSet());
    ...
    private void fillChest(Inventory chest) {
        ...
        BukkitTask task = new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < hologramLines.size(); i++) {
                    String newLine = hologramLines.get(i).replace("%t1%", String.valueOf(chestTimer)).replace("%t2%", String.valueOf(maxChestTimeSeconds));
        ...
        BukkitTask task1 = new BukkitRunnable() {
            ...
            public void run() {
                ...
                int slotsToFill = (int) (size * 0.6); // 60% от всех слотов
    ...
    private void performPotionEffects() {
        new BukkitRunnable() {
            @Override
            public void run() {
                List<Player> players = Bukkit.getOnlinePlayers().stream().filter(player -> player.getWorld() == eventWorld).collect(Collectors.toList());
                players.forEach(player -> {
                    Location l = player.getLocation();

                    Vertex2f playerPoint = new Vertex2f((float) l.getX(), (float) l.getZ());
                    if (isPointInTriangle(triangle, playerPoint)) {
                        badEffects.forEach(effectType -> player.addPotionEffect(new PotionEffect(effectType, 3, 1, true, false, false)));
                    } else if (!isPointInTriangle(triangle, playerPoint)) {
